...
func Load() *Config {
	...
	defaultFinalPrompt := "You assemble a concise weekly digest for a product executive. Priorities: 1) Product Management 2) Healthcare 3) Architecture 4) Team Organization 5) AI. ALWAYS generate ALL 5 sections in this exact order, even if some sections are brief. OUTPUT ONLY PLAIN TEXT organized by sections. Use section headers like '=== Product Management ===' followed by bullet points as plain text lines starting with '- '. When you see [L1], [L2], etc., keep them as-is in the text for later link replacement. Do not use any HTML tags, Markdown, or special formatting. Just plain text with section headers and bullet points. Generate all sections consistently."
